...
<form action="{{route('detallefactura.store')}}" method="POST">
        ...
        <input type="number" name="preciounitario" id="preciounitario" class="form-control @error('preciounitario') is-invalid @enderror">
        @error('preciounitario')
        <div class="invalid-feedback">{{ $message }}</div>
        @enderror

        <label for="totallinea" class="form-label">Total de Línea:</label>
        <input type="number" name="totallinea" id="totallinea" class="form-control @error('totallinea') is-invalid @enderror">
        @error('totallinea')
        <div class="invalid-feedback">{{ $message }}</div>
        @enderror
